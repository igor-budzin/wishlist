...
- **Database**: PostgreSQL
- **Authentication**: OAuth 2.0 (Google, Facebook, GitHub) with JWT tokens <!-- Apple temporarily disabled -->
- **AI Integration**: OpenAI GPT-4o Mini for product extraction from URLs
- **Web Scraping**: Cheerio (fast) + Playwright (SPA fallback)
- **Token Management**: JWT access/refresh tokens with database-backed revocation
...
# JWT Authentication (IMPORTANT: Generate a secure secret!)
...
The application uses OAuth 2.0 authentication with JWT token-based authentication. Users can login with Google, Facebook, or GitHub. After successful OAuth authentication, the backend issues a JWT access token (15-minute expiry) and refresh token (30-day expiry).
...
- Refresh tokens: 30-day expiry, stored in localStorage with database-backed revocation
- Automatic token refresh on expiry via Authorization header
- Logout immediately revokes refresh token in database

### Security Features

- ✅ JWT tokens with short access token expiry (15 min)
- ✅ Refresh token revocation via database tracking
- ✅ HMAC-SHA256 (HS256) token signing
...
**Note**: All wishlist endpoints require authentication. Include `Authorization: Bearer <access_token>` header with requests.
...
2. Check that JWT_SECRET is set (minimum 32 characters)
3. Verify OAuth provider credentials are correct
4. Check backend logs for detailed error messages

**"Token not working / 401 Unauthorized"**

1. Ensure access token is included in Authorization header: `Bearer <token>`
2. Check that token hasn't expired (access tokens expire after 15 minutes)
3. Use refresh token to get a new access token via `/api/auth/refresh`
4. Check browser console for token refresh errors

**"Cannot access wishlist items"**

1. Ensure you're logged in (check `/api/auth/me` with Authorization header)
...
- ✅ Authentication and user management (OAuth with 3 providers)
- ✅ User-specific wishlists with isolation
- ✅ JWT token management with refresh tokens
- ✅ Unit tests for auth services (Vitest)
